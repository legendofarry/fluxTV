    ...
    catChannels.forEach((ch) => {
      ...
      row.innerHTML = `
        <div class="flex items-center space-x-3 overflow-hidden">
            ${
              ch.logo
                ? `<img src="${ch.logo}" class="w-8 h-8 object-contain" loading="lazy" referrerpolicy="no-referrer"
                   onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex';"/>
                   <div class="w-8 h-8 hidden items-center justify-center rounded bg-slate-800 text-xs font-bold text-slate-300">
                   ${ch.name.charAt(0).toUpperCase()}</div>`
                : `<div class="w-8 h-8 flex items-center justify-center rounded bg-slate-800 text-xs font-bold text-slate-300">
                   ${ch.name.charAt(0).toUpperCase()}</div>`
            }
